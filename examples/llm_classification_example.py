...
def basic_classification_example():
    ...
    if os.getenv('ANTHROPIC_API_KEY'):
        ...
        result_claude = llm_classify_news(
            df,
            col='text',
            provider='claude'
        )
    ...
    if os.getenv('OPENAI_API_KEY'):
        ...
        result_openai = llm_classify_news(
            df,
            col='text',
            provider='openai'
        )
...
def batch_processing_example():
    ...
    for i in range(20):
        ...
        articles.append({
            'id': i + 1,
            'text': f"{headline}. " + "Additional context " * 20
        })
    ...
    if os.getenv('ANTHROPIC_API_KEY'):
        ...
        for cat in category_counts.index:
            avg_conf = result[result['llm_category_claude'] == cat]['llm_confidence_claude'].mean()
