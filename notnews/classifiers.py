...
MODEL_CONFIGS = {
    "us": {
        ...
        "soft_news_categories": [
            "Arts", "Books", "Classifieds", "Dining", "Leisure", 
...
def _load_model(region: str) -> tuple:
    ...
    try:
        ...
        raise RuntimeError(f"Failed to load model for {region}: {e}")


def classify_by_url(df: pd.DataFrame, url_col: str = "url", region: str = "us") -> pd.DataFrame:
...
def predict_soft_news(df: pd.DataFrame, text_col: str = "text", region: str = "us") -> pd.DataFrame:
    """
    Predict soft news probability using ML models.
    
    Args:
        df: DataFrame containing text
        text_col: Column name containing text
        region: "us" or "uk" for region-specific models
        
    Returns:
        DataFrame with additional column:
        - prob_soft_news: Predicted probability of soft news
    """
    ...
    # Prepare text
    result_df = df.copy()
    text_data = result_df[text_col].apply(lambda x: clean_text(x) if pd.notnull(x) else "")
...
def predict_news_category(df: pd.DataFrame, text_col: str = "text") -> pd.DataFrame:
    ...
    try:
        ...
        raise RuntimeError(f"Failed to load US category model: {e}")
