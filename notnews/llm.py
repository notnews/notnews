...
DEFAULT_CATEGORIES = {
    "hard_news": {
        "description": "Serious journalism covering politics, economics, international affairs, war, policy, and important societal issues",
        "examples": ["Election coverage", "Economic policy", "International relations", "War reporting", "Government decisions"]
    },
    "soft_news": {
        "description": "Entertainment, lifestyle, sports, celebrity coverage, and human interest stories",
        "examples": ["Celebrity gossip", "Sports scores", "Fashion trends", "Food reviews", "Travel destinations"]
    },
    "opinion": {
        "description": "Editorial content, opinion pieces, analysis, and commentary",
        "examples": ["Editorial opinions", "Op-ed columns", "Political commentary", "Cultural criticism"]
    },
    "feature": {
        "description": "In-depth reporting, investigative journalism, and long-form articles",
        "examples": ["Investigative reports", "Long-form journalism", "Profile pieces", "Data journalism"]
    },
}
...
def _parse_llm_response(response_text: str, categories: dict) -> dict[str, Any]:
    ...
    try:
        ...
        if result["category"] not in categories:
            logger.warning(f"Invalid category: {result['category']}, using first available")
...
def _classify_with_claude(text: str, categories: dict, api_key: str, model: str = "claude-3-haiku-20240307") -> dict[str, Any]:
...
def _classify_with_openai(text: str, categories: dict, api_key: str, model: str = "gpt-3.5-turbo") -> dict[str, Any]:
    """Classify text using OpenAI."""
    if not HAS_OPENAI:
        raise ImportError("openai package required for OpenAI. Install with: pip install openai")
    ...
    try:
        response = client.chat.completions.create(
            model=model,
            messages=[
                {"role": "system", "content": "You are a news categorization expert. Always respond with valid JSON."},
                {"role": "user", "content": prompt}
            ],
            temperature=0.3,
            max_tokens=500,
            response_format={"type": "json_object"} if "gpt-4" in model or "3.5-turbo-1106" in model else None
        )
...
def classify_news(
    ...
    Example:
        >>> import pandas as pd
        >>> import notnews
        >>> df = pd.DataFrame({"text": ["Election results announced", "Celebrity wedding"]})
    ...
    if not api_key:
        raise ValueError(f"No API key provided for {provider}. Set environment variable or pass api_key parameter.")
