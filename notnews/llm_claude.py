...
class ClaudeNewsClassifier(LLMNewsClassifier):
    ...
    def validate_api_key(self) -> bool:
        ...
        if not HAS_ANTHROPIC:
            logger.error(
                "anthropic package not installed. Cannot use Claude classifier."
            )
            return False
    ...
    def _classify_text(self, text: str, categories: Dict[str, Dict]) -> Dict[str, Any]:
        ...
        prompt = self.prompt_template.format(categories=categories_str, content=text)
        ...
        try:
            ...
            try:
                ...
                if not all(
                    k in result for k in ["category", "confidence", "reasoning"]
                ):
                ...
                if result["category"] not in categories:
                    logger.warning(
                        f"Claude returned invalid category: {result['category']}"
                    )
