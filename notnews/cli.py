...
@click.option("--output", "-o", default="url_classification.csv", 
              help="Output CSV file")
@click.option("--url-col", default="url", 
              help="Column containing URLs")
@click.option("--region", type=click.Choice(["us", "uk"]), default="us",
...
def classify_urls(input_file, output, url_col, region, verbose):
    ...
    try:
        ...
        if url_col not in df.columns:
            click.echo(f"Error: Column '{url_col}' not found. Available: {', '.join(df.columns)}", err=True)
            sys.exit(1)
        ...
        click.echo(f"Classified {hard_count} hard news, {soft_count} soft news articles")
...
@cli.command()
@click.argument("input_file", type=click.Path(exists=True))
@click.option("--output", "-o", default="ml_prediction.csv",
              help="Output CSV file")
...
def predict_ml(input_file, output, text_col, region, verbose):
    ...
    try:
        ...
        if text_col not in df.columns:
            click.echo(f"Error: Column '{text_col}' not found. Available: {', '.join(df.columns)}", err=True)
            sys.exit(1)
        ...
        click.echo(f"Generated {valid_predictions} predictions, average soft news probability: {avg_prob:.3f}")
...
@cli.command()
@click.argument("input_file", type=click.Path(exists=True))
@click.option("--output", "-o", default="category_prediction.csv",
              help="Output CSV file")
@click.option("--text-col", default="text", 
              help="Column containing text")
@click.option("--verbose", "-v", is_flag=True, help="Verbose output")
...
@cli.command()
@click.argument("input_file", type=click.Path(exists=True))
@click.option("--output", "-o", default="llm_classification.csv",
              help="Output CSV file")
@click.option("--text-col", default="text",
              help="Column containing text")
...
@cli.command()
@click.argument("input_file", type=click.Path(exists=True))
@click.option("--output", "-o", default="combined_classification.csv",
              help="Output CSV file")
...
def classify_all(input_file, output, text_col, url_col, region, verbose):
    ...
    try:
        ...
        click.echo(f"URL classification: {hard_count} hard news, {soft_count} soft news")
